$(function(){
	...
	function articleApi() {
		$.ajax({
			...
			success:function(data){
//			console.log(data);
				$('#title').text(data.data.title.slice(0,10));
				
				$('#txt').html(data.data.synopsis);
				txtMore(data.data.synopsis);
				$('#poster').attr('src',data.data.lookPicUrl);
				if(classify !== '2'){
					//$('#poster').attr('src',data.data.lookPicUrl);
					$('#mv').attr('src',data.data.materialUrl);
					$('#title2').text(data.data.title.slice(0,6));
					$('#mvImg').attr('src',data.data.lookPicUrl);
				}				
			},
			error:function(error){
				console.log(error);	
